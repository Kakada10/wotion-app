pipeline {
	...
	stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-13-212-199-137.ap-southeast-1.compute.amazonaws.com:8080/job/Wotion-App-CD-Prod/buildWithParameters?token=gitops-token'"
                }
            }
        }
    }
